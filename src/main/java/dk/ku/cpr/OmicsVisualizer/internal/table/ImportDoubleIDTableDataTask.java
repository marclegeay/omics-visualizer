...
//		}
	...
	public void run(TaskMonitor taskMonitor) throws Exception {
		...
		if (whereImportTable.getSelectedValue().matches(NETWORK_COLLECTION)) {
			// TODO ML : take into account several networks
		} else if (whereImportTable.getSelectedValue().matches(NETWORK_SELECTION)) {
			// TODO ML ; take into account the network and use the mapping key (see: getJoinTargetColumn)
		} else if (whereImportTable.getSelectedValue().matches(UNASSIGNED_TABLE)) {
			addTable();
		}
		*/
		// TODO ML : This is temporary and should be removed!
		addTable();
//		if (whereImportTable.getSelectedValue().matches(NETWORK_COLLECTION))
