package dk.ku.cpr.OmicsVisualizer.internal.model;

import java.util.ArrayList;

public class OmicsVisualizerShared {
	
	public static final String CYTOPANEL_NAME = "dk.ku.cpr.OmicsVisualizer.customPanel";
	
	public static final String MAPPING_CUSTOM_COLID_NAME = "dk.ku.cpr.OmicsVisualizer.internalID";
	
	public static final String MAPPING_NODE_CUSTOM_COL = "mapping_node_custom";
	public static final String MAPPING_CUSTOM_NODE_COL = "mapping_custom_node";
	public static final String CUSTOM_SUID_COL = "custom_suid";

	public static final String CYPROPERTY_NAME = "dk.ku.cpr.OmicsVisualizer";
	public static final String PROPERTIES_TABLE_SUID = "dk.ku.cpr.OmicsVisualizer.TableSUID";
	
	// We forbid the class to have instances
	private OmicsVisualizerShared() {
	}
}
